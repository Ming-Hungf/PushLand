...
namespace BLL.Controller
{
    ...
    public class ProjectController : OverallController
    {
        ...
        {
            var data = _context.Project.AsQueryable().Where(x => x.Status == EnumDefaultStatus.ACTIVE&&x.Validate==EnumValidate.VALIDATED)
        ...
        public IActionResult Update([FromForm]ProjectObject obj)
        {
            ...
            if (obj.Files != null)
            {
                var old_documents = _context.Project_Document.AsQueryable().Where(x => x.Status == EnumDefaultStatus.ACTIVE && x.ProjectID == project.ID).ToList();
