...
namespace BLL.Areas.Admin.Controller
{
    ...
    public class NewsController : OverallController
    {
        ...
        {
            var data = _context.News.AsQueryable().Where(x => x.Status == EnumDefaultStatus.ACTIVE && (x.Title.Contains(keyword) || x.Content.Contains(keyword)))
        ...
        {
            var news = _context.News.AsQueryable().Where(x=>x.ID == id&&x.Status==EnumDefaultStatus.ACTIVE).Include(x => x.Images.Where(x => x.Status == EnumDefaultStatus.ACTIVE))
        ...
        public IActionResult Update([FromForm]NewsObject obj)
        {
            ...
            var images = _context.News_Image.AsQueryable().Where(x => x.Status == EnumDefaultStatus.ACTIVE && x.NewsID == news.ID).ToList();
            if (images.Count > 0)
            {
