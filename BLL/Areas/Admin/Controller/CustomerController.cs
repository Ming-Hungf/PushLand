...
namespace BLL.Areas.Admin.Controller
{
    ...
    public class CustomerController : OverallController
    {
        ...
        {
            var data = _context.Customer.Where(x => x.CustomerType == EnumCustomer.ADVISE&&(x.Fullname.Contains(keyword)||x.Email.Contains(keyword)||x.Phone.Contains(keyword)||x.Note.Contains(keyword))).OrderByDescending(x => x.Status).ToList();
